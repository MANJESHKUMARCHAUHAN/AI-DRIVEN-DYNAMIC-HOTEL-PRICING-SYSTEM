...
class TestProductionSafety:
    ...
    def test_a_fully_configured_production_boots(
        self, monkeypatch: pytest.MonkeyPatch, isolated_env: None
    ) -> None:
        """The positive case, so the guards cannot pass by rejecting everything.

        Every negative test below asserts that some omission is refused. Without
        this one, a validator that raised unconditionally would still be green.
        """
        monkeypatch.setenv("ENVIRONMENT", "production")
        monkeypatch.setenv("POSTGRES_PASSWORD", "a-real-generated-secret")
        monkeypatch.setenv("SECURITY_ENABLED", "true")
        monkeypatch.setenv("SECURITY_READ_KEY", "a-real-read-key")
        monkeypatch.setenv("SECURITY_WRITE_KEY", "a-different-write-key")
        reset_settings()
    ...
    def test_production_refuses_unauthenticated(
        self, monkeypatch: pytest.MonkeyPatch, isolated_env: None
    ) -> None:
        """An open pricing API on the internet exposes the commercial audit
        trail and lets anyone trigger retraining."""
        monkeypatch.setenv("ENVIRONMENT", "production")
        monkeypatch.setenv("POSTGRES_PASSWORD", "a-real-generated-secret")
        monkeypatch.setenv("SECURITY_ENABLED", "false")
        reset_settings()
        with pytest.raises(ValidationError, match="SECURITY_ENABLED"):
            get_settings()

    def test_production_refuses_the_shipped_keys(
        self, monkeypatch: pytest.MonkeyPatch, isolated_env: None
    ) -> None:
        monkeypatch.setenv("ENVIRONMENT", "production")
        monkeypatch.setenv("POSTGRES_PASSWORD", "a-real-generated-secret")
        monkeypatch.setenv("SECURITY_ENABLED", "true")
        reset_settings()
        with pytest.raises(ValidationError, match="SECURITY_READ_KEY"):
            get_settings()

    def test_production_refuses_identical_scopes(
        self, monkeypatch: pytest.MonkeyPatch, isolated_env: None
    ) -> None:
        """Identical keys collapse the two scopes and hand every reader write
        access -- which would silently undo the agent's read-only guarantee."""
        monkeypatch.setenv("ENVIRONMENT", "production")
        monkeypatch.setenv("POSTGRES_PASSWORD", "a-real-generated-secret")
        monkeypatch.setenv("SECURITY_ENABLED", "true")
        monkeypatch.setenv("SECURITY_READ_KEY", "same-key-for-both")
        monkeypatch.setenv("SECURITY_WRITE_KEY", "same-key-for-both")
        reset_settings()
