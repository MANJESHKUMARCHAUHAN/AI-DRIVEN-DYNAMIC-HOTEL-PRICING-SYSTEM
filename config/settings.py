...
class KafkaSettings(BaseSettings):
    ...
    request_timeout_ms: int = Field(default=40_000, ge=1_000)
...
class IngestionSettings(BaseSettings):
    ...
    synthetic_hotels: int = Field(default=5, ge=1)
    ...
    def _guard_real_scrapers(self) -> "IngestionSettings":
        if self.source is not CompetitorSource.SYNTHETIC and not self.enable_real_scrapers:
            raise ValueError(
                f"INGESTION_SOURCE={self.source.value} requires "
                "INGESTION_ENABLE_REAL_SCRAPERS=true. Real scrapers are disabled "
                "by default; the supported default is INGESTION_SOURCE=synthetic."
            )
        return self
...
class Settings(BaseSettings):
    ...
    def _production_safety(self) -> "Settings":
        """Refuse to boot in production with the shipped development password."""
